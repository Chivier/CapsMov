...
@Test func quickCapsLockTapTogglesOriginalCapsLock() {
  ...
  #expect(
    engine.handle(.capsLock(isDown: true, device: nil, timeMilliseconds: 1_000)) == .consume())
  #expect(
    engine.handle(.capsLock(isDown: false, device: nil, timeMilliseconds: 1_120))
      == consumeTogglingCapsLock())
}
...
@Test func longCapsLockHoldClearsInsteadOfTogglingCapsLock() {
  ...
  #expect(
    engine.handle(.capsLock(isDown: true, device: nil, timeMilliseconds: 1_000)) == .consume())
  #expect(
    engine.handle(.capsLock(isDown: false, device: nil, timeMilliseconds: 1_250))
      == consumeClearingCapsLock())
}
...
@Test func capsLockWithMappedShortcutClearsAndDoesNotToggleBeforeThreshold() {
  ...
  #expect(
    engine.handle(.capsLock(isDown: true, device: nil, timeMilliseconds: 1_000)) == .consume())
  #expect(
    engine.handle(.key(.e, phase: .down, modifiers: [], device: nil, timeMilliseconds: 1_030))
      == .consume(
        [
          .key(.upArrow, phase: .down, modifiers: [])
        ], actions: [.clearCapsLock]))
  #expect(
    engine.handle(.key(.e, phase: .up, modifiers: [], device: nil, timeMilliseconds: 1_040))
      == .consume([
        .key(.upArrow, phase: .up, modifiers: [])
      ]))
  #expect(
    engine.handle(.capsLock(isDown: false, device: nil, timeMilliseconds: 1_050))
      == consumeClearingCapsLock())
}
...
@Test func capsLockWithUnmappedKeyClearsAndDoesNotToggle() {
  ...
  #expect(
    engine.handle(
      .key(.other(99), phase: .down, modifiers: [], device: nil, timeMilliseconds: 1_030))
      == passThroughClearingCapsLock())
  #expect(
    engine.handle(.key(.other(99), phase: .up, modifiers: [], device: nil, timeMilliseconds: 1_040))
      == .passThrough)
  #expect(
    engine.handle(.capsLock(isDown: false, device: nil, timeMilliseconds: 1_050))
      == consumeClearingCapsLock())
...
@Test func capsLockEMapsToUpArrowAcrossKeyboardTransports() {
  for transport in KeyboardTransport.allCases {
    ...
    #expect(
      engine.handle(.key(.e, phase: .down, modifiers: [], device: device))
        == consumeClearingCapsLock([
          .key(.upArrow, phase: .down, modifiers: [])
        ]))
    #expect(
      engine.handle(.key(.e, phase: .up, modifiers: [], device: device))
        == .consume([
          .key(.upArrow, phase: .up, modifiers: [])
        ]))
...
@Test func lineNavigationUsesMacCommandArrowSemantics() {
  ...
  #expect(engine.handle(.capsLock(isDown: true, device: nil)) == .consume())
  #expect(
    engine.handle(.key(.j, phase: .down, modifiers: [], device: nil))
      == consumeClearingCapsLock([
        .key(.leftArrow, phase: .down, modifiers: [.command])
      ]))
  #expect(
    engine.handle(.key(.j, phase: .up, modifiers: [], device: nil))
      == .consume([
        .key(.leftArrow, phase: .up, modifiers: [.command])
      ]))
  #expect(
    engine.handle(.key(.l, phase: .down, modifiers: [.shift], device: nil))
      == .consume([
...
@Test func everyCapsLockMappingUsesExpectedOutputKey() {
  ...
  for (input, output) in cases {
    ...
    #expect(engine.handle(.capsLock(isDown: true, device: nil)) == .consume())
    #expect(
      engine.handle(.key(input, phase: .down, modifiers: [], device: nil))
        == consumeClearingCapsLock([output]))
  }
}
...
@Test func mappedKeysPreserveShortcutModifiers() {
  ...
  #expect(engine.handle(.capsLock(isDown: true, device: nil)) == .consume())
  #expect(
    engine.handle(.key(.e, phase: .down, modifiers: [.shift], device: nil))
      == consumeClearingCapsLock([
        .key(.upArrow, phase: .down, modifiers: [.shift])
      ]))
  #expect(
    engine.handle(.key(.d, phase: .down, modifiers: [.command, .option], device: nil))
      == .consume([
        .key(.downArrow, phase: .down, modifiers: [.command, .option])
      ]))
  #expect(
    engine.handle(.key(.j, phase: .down, modifiers: [.shift, .control], device: nil))
      == .consume([
...
@Test func unmappedKeysPassThroughWhileCapsLockIsHeld() {
  ...
  #expect(engine.handle(.capsLock(isDown: true, device: nil)) == .consume())
  #expect(
    engine.handle(.key(.other(99), phase: .down, modifiers: [], device: nil))
      == passThroughClearingCapsLock())
...
@Test func releasingCapsLockBeforeMappedKeyReleasesSynthesizedKeyUp() {
  ...
  #expect(engine.handle(.capsLock(isDown: true, device: nil)) == .consume())
  #expect(
    engine.handle(.key(.e, phase: .down, modifiers: [], device: nil))
      == consumeClearingCapsLock([
        .key(.upArrow, phase: .down, modifiers: [])
      ]))
  #expect(
    engine.handle(.capsLock(isDown: false, device: nil))
      == consumeClearingCapsLock([
        .key(.upArrow, phase: .up, modifiers: [])
      ]))
...
@Test func multipleMappedKeysHeldUntilCapsLockReleaseAreReleasedAndConsumed() {
  ...
  #expect(
    engine.handle(.key(.d, phase: .down, modifiers: [.shift], device: nil))
      == .consume([
...
@Test func multipleShortcutsWithinOneCapsLockHoldDoNotLeakState() {
  ...
  #expect(
    engine.handle(.key(.e, phase: .up, modifiers: [], device: nil))
      == .consume([
        .key(.upArrow, phase: .up, modifiers: [])
      ]))
  #expect(
    engine.handle(.key(.f, phase: .down, modifiers: [], device: nil))
      == .consume([
        .key(.rightArrow, phase: .down, modifiers: [])
      ]))
  #expect(
    engine.handle(.key(.f, phase: .up, modifiers: [], device: nil))
      == .consume([
...
@Test func allShortcutsCanRunSequentiallyWithinOneCapsLockHold() {
  ...
  for (index, entry) in cases.enumerated() {
    ...
    #expect(
      engine.handle(.key(input, phase: .up, modifiers: [], device: nil)) == .consume([outputUp]))
  }
  #expect(engine.handle(.capsLock(isDown: false, device: nil)) == consumeClearingCapsLock())
...
@Test func uppercaseMappedLettersWithoutCapsHoldPassThrough() {
  ...
  for key in [Key.s, .e, .d, .f] {
    #expect(
      engine.handle(.key(key, phase: .down, modifiers: [.shift], device: nil)) == .passThrough)
...
@Test func physicalCapsLockReleasePreventsLaterUppercaseRemaps() {
  ...
  #expect(engine.handle(.capsLock(isDown: true, device: nil)) == .consume())
  #expect(
    engine.handle(.key(.s, phase: .down, modifiers: [], device: nil))
      == consumeClearingCapsLock([
        .key(.leftArrow, phase: .down, modifiers: [])
      ]))
  #expect(
    engine.handle(.key(.s, phase: .up, modifiers: [], device: nil))
      == .consume([
        .key(.leftArrow, phase: .up, modifiers: [])
      ]))
