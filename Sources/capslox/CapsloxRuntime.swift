...
let generatedEventMarker: Int64 = 0x6361_7073_6c6f_78
private let defaultCapsLockTapThresholdMilliseconds: UInt64 = 250
private let tapThresholdDefaultsKey = "CapsMovTapThresholdMilliseconds"

final class CapsloxRuntime {
  private(set) var capsLockTapThresholdMilliseconds: UInt64
  ...
  func start() -> Bool {
    ...
    let eventTap = CapsloxEventTap(
      capsLockTapThresholdMilliseconds: capsLockTapThresholdMilliseconds)
  ...
  @discardableResult
  func updateCapsLockTapThresholdMilliseconds(_ milliseconds: UInt64) -> Bool {
    guard milliseconds != capsLockTapThresholdMilliseconds else {
      return isRunning
    }

    let wasEnabled = isEnabled
    stop()
    capsLockTapThresholdMilliseconds = milliseconds
    UserDefaults.standard.set(Int(milliseconds), forKey: tapThresholdDefaultsKey)

    let started = start()
    if started, !wasEnabled {
      setEnabled(false)
    }
    return started
  }
...
private final class CapsloxEventTap {
  ...
  private func post(_ output: OutputEvent) {
    ...
    guard
      let event = CGEvent(
        keyboardEventSource: nil, virtualKey: CGKeyCode(keyCode), keyDown: keyDown)
    else {
      return
    }
...
private final class CapsLockLockStateController {
  ...
  func start() -> Bool {
    ...
    return IOServiceOpen(service, mach_task_self_, UInt32(kIOHIDParamConnectType), &connection)
      == KERN_SUCCESS
  }
...
private final class CapsLockPhysicalMonitor {
  ...
  func start() -> Bool {
    ...
    IOHIDManagerRegisterInputValueCallback(
      manager, capsLockInputValueCallback, Unmanaged.passUnretained(self).toOpaque())
...
func configuredCapsLockTapThresholdMilliseconds() -> UInt64 {
  if let rawValue = ProcessInfo.processInfo.environment["CAPSLOX_TAP_THRESHOLD_MS"],
  ...
  let storedThreshold = UserDefaults.standard.integer(forKey: tapThresholdDefaultsKey)
  return storedThreshold > 0 ? UInt64(storedThreshold) : defaultCapsLockTapThresholdMilliseconds
}
