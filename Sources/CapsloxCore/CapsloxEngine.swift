...
public struct InputEvent: Equatable {
  ...
  public static func capsLock(isDown: Bool, device: KeyboardDevice?, timeMilliseconds: UInt64 = 0)
    -> InputEvent
  {
    InputEvent(
      key: .capsLock,
...
public enum EventDecision: Equatable {
  ...
  public static func consume(_ output: [OutputEvent] = [], actions: [SystemAction] = [])
    -> EventDecision
  {
...
public final class CapsloxEngine {
  ...
  private func handleCapsLock(phase: KeyPhase, timeMilliseconds: UInt64) -> EventDecision {
    ...
    case .up:
      ...
      let releaseEvents = releaseActiveRemaps()
      let shouldToggleCapsLock =
        !capsLockWasUsedAsModifier && isWithinTapThreshold(upAt: timeMilliseconds)
      isCapsLockDown = false
      capsLockDownAtMilliseconds = nil
      capsLockWasUsedAsModifier = false
      return .consume(
