...
public enum CapsloxPresentation {
  ...
  public static func secureInputDetail(for status: SecureInputStatus) -> String {
    if let owner = status.owner {
      return
        "\(owner.name) is using macOS Secure Input, so CapsMov can't read keys for now. This is expected while a password field is focused—move focus out of it (or quit \(owner.name)) and CapsMov resumes on its own."
    }
    if status.isStaleProcess {
      return
        "macOS is still blocking keyboard monitoring for pid \(status.pid), even though that process has exited. Reset safely locks your Mac; unlock once and CapsMov will recheck automatically."
    }
    return
  ...
  public static func secureInputActionTitle(for status: SecureInputStatus) -> String {
    ...
    return status.isStaleProcess ? "Reset…" : "Refresh"
  }
}
